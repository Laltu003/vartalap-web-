...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(
      ...
      (err) => {
        console.error('AdminPage users listener error:', err);
        setLoading(false);
      }
    );
    return unsub;
  ...
  useEffect(() => {
    ...
    const unsub = onValue(
      ...
      (err) => console.error('AdminPage chats listener error:', err)
    );
    return unsub;
  }, []);
      ...
      <nav className="admin-sidebar">
        <div className="admin-sidebar-logo">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <img src={logo} alt="VartaLap" style={{ width: 30, height: 30, borderRadius: 7 }} />
            <h2 style={{ margin: 0 }}>VartaLap</h2>
          </div>
          <p>Admin Panel</p>
        </div>
